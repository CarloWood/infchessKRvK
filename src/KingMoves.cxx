...
KingMoves::KingMoves(Board const& board, Color color, bool forward)
{
  ...
  for (int dx = -1; dx <= 1; ++dx)
  {
    ...
    for (int dy = -1; dy <= 1; ++dy)
    {
      ...
      if (color == black && adjacent_board.white_rook() == adjacent_king_square && !adjacent_king_square.is_next_to(adjacent_board.white_king()))
      {
      ...
      if (!adjacent_board.determine_legal(color))
        continue;
